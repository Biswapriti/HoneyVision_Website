...
const AdminDashboard = () => {
  ...
  const changeUserRole = async (userId, role) => {
    try {
      await authApi.request(`/admin/users/${userId}/role`, {
        method: "PUT",
          ...
          <div className="space-y-6">
            <div className="grid gap-6 xl:grid-cols-4 lg:grid-cols-2">
              ...
              >
                ...
                <p className="mt-4 text-4xl font-bold text-white">{Math.max(0, (summary?.contactCount ?? 0) - 2)}</p>
                <p className="mt-2 text-sm text-gray-400">Requested demos this week</p>
              </button>
              <button
                type="button"
                      ...
                      <tr key={userItem._id}>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-wrap gap-2">
                            <button
                              type="button"
                              onClick={() => changeUserRole(userItem._id, userItem.role === 'admin' ? 'user' : 'admin')}
